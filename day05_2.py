...
def get_mapped_range_from_single_range(input_range: list[int], map: list) -> list:
    ...
    if input_range[1] < source_range[0] or input_range[0] > source_range[1]:
        ...
        mapped_range.append([input_range[0] - map[1] + map[0], input_range[1] - map[1] + map[0]])
        return mapped_range
    elif input_range[0] >= source_range[0] and input_range[1] > source_range[1]:
        mapped_range.append([input_range[0] - map[1] + map[0], source_range[1] - map[1] + map[0]])
        mapped_range.append([source_range[1]+1, input_range[1]])
        return mapped_range
    elif input_range[0] < source_range[0] and input_range[1] <= source_range[1]:
        mapped_range.append([input_range[0], source_range[0] - 1])
        mapped_range.append([source_range[0] - map[1] + map[0], input_range[1] - map[1] + map[0]])
        return mapped_range
    else:
        mapped_range.append([input_range[0], source_range[0] - 1])
        mapped_range.append([source_range[0] - map[1] + map[0], source_range[1] - map[1] + map[0]])
        mapped_range.append([source_range[1] + 1, input_range[1]])
        return mapped_range
